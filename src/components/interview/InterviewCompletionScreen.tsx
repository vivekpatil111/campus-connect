  ...
  const generatePDFReport = async () => {
    ...
    try {
      ...
      const pdfContent = `
        ...
        Duration: ${Math.round(answers.reduce((total, answer) => total + answer.timeTaken, 0) / 1000)} seconds
        ...
        - Engagement: ${calculateEngagementScore()}%
        - Overall: ${calculateOverallScore()}%
        - Confidence Level: ${getConfidenceLevel(calculateOverallScore())}

        Question Analysis:
        ${answers.slice(0, 5).map((answer, index) => `
          Q${index + 1}: ${answer.question.substring(0, 50)}...
          Answer Length: ${answer.transcript.length} characters
          Time Taken: ${answer.timeTaken} seconds
        `).join('\n\n')}

        Engagement Metrics:
        - Camera Active: ${(engagementMetrics.cameraOnDuration / engagementMetrics.totalDuration * 100).toFixed(1)}%
        - Face Present: ${(engagementMetrics.faceDetected / engagementMetrics.totalDuration * 100).toFixed(1)}%
        - Speaking Time: ${(engagementMetrics.speakingTime / engagementMetrics.totalDuration * 100).toFixed(1)}%
      ...
      a.href = url;
      a.download = `Interview_Report_${company}_${role}_${interviewType}_${new Date().toISOString().slice(0, 10)}.txt`;
